...
class ModelTraining:
    ...
    def load_and_split_data(self):
        try:
            ...
            logger.info('loading booking status')
            X_train = train_df.drop(columns = ['booking_status'])
            y_train = train_df['booking_status']
            logger.info('loaded train dataframe')

            X_test = test_df.drop(columns=['booking_status'])
            y_test = test_df['booking_status']
            logger.info('loaded test dataframe')
            return X_train,y_train,X_test,y_test
        except Exception as e:
            logger.error(f'Error while loading data {e}')
            raise CustomException(f'erorr while loading data',e)
    def train_lgbm(self,X_train,y_train):
        try:
            logger.info('Initiating our model')

            lgbm_model = lgb.LGBMClassifier(random_state=self.random_search['random_state'])
            ...
            return best_lgbm_model
                ...
                raise CustomException(f'erorr while training model',e)
